...
- 2–10 answer options, optional expiry (1 hour / 1 day / 1 week)
- Unique share link (`/p/<slug>`) plus QR code
- One vote per browser session, enforced in the database
- Live results chart driven by Supabase Realtime, with reconnect reconciliation
...
Typed RPC errors (`already_voted`, `poll_closed`, `rate_limited`, …) are mapped
to user-friendly messages in `lib/errors.ts`. Inputs are validated with Zod in
Server Actions and re-checked with CHECK constraints in PostgreSQL.
...
Deploy to any Next.js host (e.g. Vercel). Configure the two environment
variables above and point `NEXT_PUBLIC_SUPABASE_URL` at your production
Supabase project. The corporate-proxy shim in `lib/supabase/fetch.ts` activates
only when `HTTP(S)_PROXY` variables exist and is inert in production.
